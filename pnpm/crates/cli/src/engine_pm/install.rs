...
//! `PATH`.
//!
//! The slot is entered only under its [`crate::slot_lock`]. When another
//! process holds that lock, the engine is installed into a
...
use pnpm_store_dir::{PrivateInstall, StoreDir};
...
/// An engine installed and ready to run.
...
/// Install the `pm` engine for `version` into the global virtual store, or
/// privately when its slot is held, and return where its bins are linked.
///
...
async fn install_engine_into_slot<Reporter: self::Reporter + 'static>(
    ...
    let tmp_install_dir =
        config.store_dir.unique_tmp_dir(&format!("{}-engine-{version}", pm.name()));
    ...
    link_engine_bins(&slot?, package_name, package.links_native_binary)
}

/// Install the engine into a private directory of this process's own,
/// self-contained rather than linked into the global virtual store, for
/// when another process holds the slot lock.
async fn install_engine_privately<Reporter: self::Reporter + 'static>(
    config: &'static Config,
    pm: PackageManager,
    version: &str,
    package: EnginePackages,
...
/// another process holds it: the engine is then installed privately. A
/// lock that cannot be established at all is reported and treated the
/// same, since the private install needs none.
fn engine_install_lock<Reporter: self::Reporter>(
    ...
    let path = config.store_dir
        ...
        .join("engine-locks")
        .join(name);
    match slot_lock::acquire(path.clone()) {
        Ok(lock) => lock,
        Err(error) => {
            Reporter::emit(&LogEvent::Pnpm(PnpmLog {
                level: LogLevel::Warn,
                message: format!(
                    "Could not lock the package manager engine install at {}: {error}. Installing it into a private directory instead.",
                    path.display(),
                ),
                prefix: String::new(),
            }));
...
/// Remove the private engine installs no process holds any more. The
/// engine store is not the store `pnpm store prune` sweeps, so its
/// leftovers are swept here.
pub(crate) fn remove_orphaned_private_engine_installs(config: &Config) -> miette::Result<()> {
    let Some(global_pkg_dir) = &config.global_pkg_dir else {
        return Ok(());
    };
...
/// Link the engine installed under `install_root` — its global-virtual-store
